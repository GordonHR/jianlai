...
/* =====================================================================
   落魄山 · 门派经营养成
   独立玩法模块。玩家扮演山主陈平安，以「旬」为节奏经营落魄山：
   收徒、建观、养剑葫、祭炼飞剑、应对江湖与蛮荒，在节点 deadline 与道心约束下，
   于第五十四旬「落魄山建宗大典」求得立世。

   依赖 game.js 全局：SFX、render()、state。
   本文件自包含：配置数据 / 状态 / 旬循环 / 行动 / 节点 / 结局 / 渲染 / 样式注入。
   ===================================================================== */

/* ===================== 样式注入 ===================== */
(function injectSectStyle(){
  if(document.getElementById('sect-style')) return;
  const css = `
  .sect{ color:#ece6d8; max-width:1080px; margin:0 auto; position:relative;
    background:
      radial-gradient(140% 60% at 50% 0%, rgba(232,198,106,.05), transparent 55%),
      radial-gradient(120% 120% at 50% 100%, rgba(0,0,0,.30), transparent 60%);
    border-radius:18px; box-shadow: inset 0 0 90px rgba(0,0,0,.28);
    padding:14px 6px; overflow:hidden; }
  /* 背景剑气：一道斜向冷光，长间隔掠过，只作氛围，不接交互 */
  .sect::before{
    content:''; position:absolute; top:-45%; left:-70%; width:42%; height:190%;
    background:linear-gradient(90deg, transparent, rgba(232,198,106,.05) 42%, rgba(255,244,214,.11) 50%, rgba(232,198,106,.05) 58%, transparent);
    transform:rotate(18deg); pointer-events:none; opacity:0;
    animation:sectSwordGleam 17s ease-in-out infinite;
  }
  @keyframes sectSwordGleam{
    0%,86%{ transform:translateX(0) rotate(18deg); opacity:0; }
    89%    { opacity:1; }
    100%   { transform:translateX(330%) rotate(18deg); opacity:0; }
  }
  .sect-head{ display:flex; align-items:flex-end; justify-content:space-between; flex-wrap:wrap; gap:8px; border-bottom:1px solid rgba(232,198,106,.3); padding-bottom:10px; margin-bottom:14px; }
  .sect-title{ font-family:var(--kai); font-size:30px; color:var(--gold); letter-spacing:4px; }
  .sect-title small{ font-family:inherit; font-size:13px; color:#b9ad8f; letter-spacing:1px; margin-left:10px; }
  .sect-timer{ font-size:13px; color:#cdb98a; text-align:right; }
  .sect-timer b{ color:var(--gold); font-size:18px; }
  .sect-season{ display:inline-block; padding:1px 8px; border:1px solid rgba(232,198,106,.4); border-radius:10px; margin-left:6px; }

  .sect-res{ display:flex; gap:10px; flex-wrap:wrap; margin:10px 0 6px; }
  .sect-res-i{ display:flex; align-items:center; gap:8px; background:rgba(20,17,15,.55); border:1px solid rgba(232,198,106,.22); border-radius:10px; padding:7px 12px; min-width:96px; }
  .sect-res-i .ico{ display:inline-flex; width:24px; height:24px; flex:0 0 24px; color:var(--gold); opacity:.92; }
  .sect-res-i .ico svg{ display:block; }
  .sect-res-i .k{ font-size:12px; color:#b9ad8f; }
  .sect-res-i .v{ font-size:20px; color:var(--gold); font-weight:bold; }
  .sect-res-i .v.warn{ color:#e0795f; }
  .sect-res-i .v.good{ color:#7fc79a; }

  .sect-nodes{ display:flex; gap:8px; flex-wrap:wrap; margin:6px 0 14px; }
  .sect-node{ font-size:12px; padding:4px 10px; border-radius:14px; border:1px solid rgba(232,198,106,.25); background:rgba(20,17,15,.4); color:#cdb98a; }
  .sect-node .bar{ display:inline-block; width:46px; height:5px; border-radius:3px; background:rgba(255,255,255,.12); margin-left:6px; vertical-align:middle; overflow:hidden; }
  .sect-node .bar i{ display:block; height:100%; background:var(--gold); }
  .sect-node.done{ opacity:.55; }
  .sect-node.fire{ border-color:#e0795f; color:#ffb59a; box-shadow:0 0 10px rgba(224,121,95,.4); }

  .sect-grid{ display:grid; grid-template-columns:1fr 1fr; gap:14px; }
  @media(max-width:780px){ .sect-grid{ grid-template-columns:1fr; } }
  .sect-panel{ background:rgba(20,17,15,.5); border:1px solid rgba(232,198,106,.18); border-radius:12px; padding:12px 14px; }
  .sect-panel h3{ margin:0 0 10px; font-family:var(--kai); font-size:18px; color:var(--gold); letter-spacing:2px; border-left:3px solid var(--gold); padding-left:9px; }
  .sect-empty{ color:#8a8170; font-size:13px; padding:8px 0; }

  /* 册首语：章内"节"标题 + 原著向意境短句（每进入一册弹出一次） */
  .sect-prologue{ background:linear-gradient(180deg,rgba(40,32,22,.72),rgba(20,17,15,.55)); border:1px solid rgba(232,198,106,.35); border-radius:14px; padding:18px 22px; margin:0 0 16px; position:relative; }
  .sect-prologue .pl-sub{ font-size:12px; color:#b9ad8f; letter-spacing:2px; margin-bottom:6px; }
  .sect-prologue .pl-title{ font-family:var(--kai); font-size:26px; color:var(--gold); letter-spacing:3px; margin-bottom:10px; }
  .sect-prologue .pl-body{ font-size:15px; line-height:2; color:#ece6d8; }
  .sect-prologue .pl-turn{ margin-top:14px; text-align:right; }

  .sect-card-row{ display:flex; align-items:center; justify-content:space-between; gap:8px; padding:8px 0; border-bottom:1px dashed rgba(232,198,106,.14); }
  .sect-card-row:last-child{ border-bottom:none; }
  .sect-card-name{ font-size:15px; color:#f0e8d2; }
  .sect-card-sub{ font-size:12px; color:#a99e84; margin-top:2px; }
  .sect-lv{ font-size:12px; color:var(--gold); }

  /* 立绘 */
  .sect-hero{ display:flex; align-items:center; gap:14px; margin:10px 0 4px; }
  .sect-hero .ph{ width:76px; height:108px; border-radius:10px; object-fit:cover; border:1px solid rgba(232,198,106,.45); box-shadow:0 4px 16px rgba(0,0,0,.45); background:rgba(20,17,15,.6); flex:none; }
  .sect-hero .phname{ font-family:var(--kai); font-size:20px; color:var(--gold); letter-spacing:3px; }
  .sect-hero .phsub{ font-size:12px; color:#b9ad8f; margin-top:4px; }
  .sect-portrait{ width:54px; height:76px; border-radius:8px; object-fit:cover; border:1px solid rgba(232,198,106,.35); flex:none; background:rgba(20,17,15,.6); }
  .sect-disc{ align-items:flex-start; }
  .sect-disc .dinfo{ flex:1; min-width:0; }

  .sect-btn{ font-family:inherit; cursor:pointer; border:1px solid rgba(232,198,106,.4); background:rgba(232,198,106,.08); color:var(--gold); border-radius:8px; padding:6px 12px; font-size:13px; transition:.15s; }
  .sect-btn:hover{ background:rgba(232,198,106,.2); }
  .sect-btn.primary{ background:var(--gold); color:#1a140c; font-weight:bold; border-color:var(--gold); }
  .sect-btn.primary:hover{ background:#f3d887; }
  .sect-btn.ghost{ background:transparent; color:#b9ad8f; border-color:rgba(232,198,106,.25); }
  .sect-btn:disabled{ opacity:.4; cursor:not-allowed; }
  .sect-btn.sm{ padding:4px 9px; font-size:12px; }

  .sect-ap{ font-size:13px; color:#cdb98a; margin:4px 0 12px; }
  .sect-ap b{ color:var(--gold); font-size:16px; }

  .sect-actions{ display:flex; gap:10px; flex-wrap:wrap; margin:6px 0 14px; }

  /* 事件卡 */
  .sect-events{ display:flex; flex-direction:column; gap:12px; }
  .sect-ev{ background:linear-gradient(180deg,rgba(36,30,22,.7),rgba(20,17,15,.7)); border:1px solid rgba(232,198,106,.3); border-radius:12px; padding:14px; }
  .sect-ev.urgent{ border-color:#e0795f; box-shadow:0 0 14px rgba(224,121,95,.25); }
  .sect-ev-type{ display:inline-block; font-size:11px; padding:2px 8px; border-radius:10px; margin-bottom:6px; }
  .ev-江湖{ background:rgba(47,111,94,.25); color:#8fd6bb; border:1px solid rgba(47,111,94,.5); }
  .ev-讲道理{ background:rgba(41,128,185,.22); color:#8fbce8; border:1px solid rgba(41,128,185,.5); }
  .ev-天劫{ background:rgba(192,57,43,.22); color:#e89b8f; border:1px solid rgba(192,57,43,.5); }
  .ev-奇遇{ background:rgba(212,160,23,.22); color:#e8cf8f; border:1px solid rgba(212,160,23,.5); }
  .ev-访客{ background:rgba(140,90,200,.22); color:#c9a8f0; border:1px solid rgba(140,90,200,.5); }
  .ev-论道{ background:rgba(41,128,185,.22); color:#8fbce8; border:1px solid rgba(41,128,185,.5); }
  .ev-问心{ background:rgba(192,57,43,.26); color:#f0b3a8; border:1px solid rgba(192,57,43,.62); box-shadow:0 0 10px rgba(192,57,43,.3); }
  /* 问心：朱砂批注 + 心跳呼吸，暗示「这一问没有标准答案」 */
  .sect-ev.ask{ position:relative; border-color:rgba(192,57,43,.62); box-shadow:0 0 18px rgba(192,57,43,.22); animation:sectInkIn .5s ease both, sectHeartbeat 3.2s ease-in-out .5s infinite; }
  .sect-ev.ask::before{ content:''; position:absolute; left:0; top:0; bottom:0; width:3px; background:linear-gradient(180deg,#c0392b,rgba(192,57,43,0)); border-radius:12px 0 0 12px; }
  .sect-ev.ask h4{ color:#f7d9d2; }
  .sect-ev.ask .sect-opt:not(:disabled):hover{ border-color:rgba(192,57,43,.75); box-shadow:0 0 12px rgba(192,57,43,.3); }
  @keyframes sectHeartbeat{
    0%,100%{ box-shadow:0 0 18px rgba(192,57,43,.18); }
    50%    { box-shadow:0 0 30px rgba(192,57,43,.42); }
  }
  .sect-ev h4{ margin:2px 0 6px; font-size:17px; color:#f3e8c8; font-family:var(--kai); letter-spacing:1px; }
  .sect-ev p{ margin:0 0 10px; font-size:14px; color:#cabf9f; line-height:1.6; }
  .sect-quote{ margin:0 0 12px; padding:8px 14px; border-left:3px solid rgba(232,198,106,.6); background:rgba(232,198,106,.06); border-radius:0 8px 8px 0; font-family:var(--kai); font-size:15px; color:#f3e8c8; line-height:1.6; font-style:normal; }
  /* 与山水祠 .sc-opt 同规格：padding / 字号 / 行高 / 最小高度 一致 */
  .sect-opt{ display:flex; align-items:flex-start; gap:10px; width:100%; text-align:left; margin:8px 0; padding:10px 14px; min-height:44px; border-radius:9px; border:1px solid rgba(232,198,106,.28); background:rgba(20,17,15,.5); cursor:pointer; color:#e8dec4; font-family:inherit; font-size:14px; line-height:1.5; transition:.15s; }
  .sect-opt:not(:disabled):hover{ background:rgba(232,198,106,.14); border-color:var(--gold); }
  /* 禁用态：不用整体 opacity 淡化（会连锁定原因一起吃掉），压暗底与边框即可，文字保留可读 */
  .sect-opt:disabled{ opacity:1; cursor:not-allowed; background:rgba(18,16,14,.66); border-color:rgba(232,198,106,.16); color:#9a9284; }
  /* .req 是「为什么现在不能选」的约束提示，保留；
     .note / .gain 是事前剧透后果，选项已不再渲染，随之删除。 */
  .sect-opt .req{ display:inline-block; font-size:12px; color:#e0795f; margin-left:8px; }
  /* 事件选项图标：朴素古风小章，与按钮内容左上对齐 */
  .sect-opt-ico{ display:inline-flex; align-items:center; justify-content:center; width:26px; height:26px; flex-shrink:0; margin-top:1px; border:1px solid rgba(232,198,106,.4); border-radius:50%; background:rgba(20,17,15,.55); color:#e8cf8f; }
  .sect-opt-ico svg{ width:14px; height:14px; display:block; }
  .sect-ev.urgent .sect-opt-ico{ border-color:rgba(217,139,106,.55); color:#e0916a; }
  .sect-ev.urgent .sect-opt-ico svg{ stroke:#e0916a; }
  .sect-ev.ask .sect-opt-ico{ border-color:rgba(143,188,232,.45); color:#8fbce8; }
  /* 锁定原因为约束提示（可显示），禁用时须比正文更醒目，避免被压暗后看不清 */
  .sect-opt:disabled .req{ color:#e8916a; }
  /* 搁置：与正常选项同长宽、同图标圆章，仅压暗边框/底以示「暂缓」语义 */
  .sect-opt.skip-opt{ border-color:rgba(232,198,106,.16); background:rgba(20,17,15,.38); }
  .sect-opt.skip-opt span:last-child{ color:#b9ad8f; }

  .sect-log{ font-size:12.5px; color:#b3a988; line-height:1.7; max-height:160px; overflow:auto; }
  .sect-log .lg{ padding:2px 0; border-bottom:1px dashed rgba(232,198,106,.1); }
  .sect-log .lg.good{ color:#8fd6bb; }
  .sect-log .lg.bad{ color:#e89b8f; }
  .sect-log .lg.big{ color:var(--gold); }

  .sect-back{ margin-top:18px; text-align:center; }

  /* 本旬结算中转页 */
  .sect-settle{ text-align:center; padding:44px 18px; }
  .sect-settle .st-head{ font-size:30px; letter-spacing:10px; color:var(--gold); margin-bottom:8px; }
  .sect-settle .st-sub{ font-size:13px; color:#9ed4be; letter-spacing:2px; margin-bottom:24px; }
  .sect-settle .st-ledger{ max-width:520px; margin:0 auto 18px; border:1px solid #2c3a33; background:#141c18; border-radius:2px; overflow:hidden; }
  .sect-settle .st-row{ display:flex; align-items:center; justify-content:space-between; padding:13px 22px; border-bottom:1px solid #26332c; }
  .sect-settle .st-row:last-child{ border-bottom:none; }
  .sect-settle .st-k{ font-size:14px; color:#9fb0a6; letter-spacing:2px; }
  .sect-settle .st-v{ font-size:17px; font-weight:bold; color:#b9ad8f; font-family:"KaiTi","STKaiti",serif; }
  .sect-settle .st-row.up .st-v{ color:#8fc49b; }
  .sect-settle .st-row.down .st-v{ color:#d98b6a; }
  .sect-settle .st-notes{ max-width:560px; margin:0 auto 14px; }
  .sect-settle .st-note{ font-size:12px; color:#8fa098; line-height:1.8; font-style:italic; }
  .sect-settle .st-backlog{ font-size:12px; color:#d98b6a; margin-bottom:18px; }
  ...
  .sect-settle .st-head{ animation:stSeal .56s cubic-bezier(.2,.8,.3,1) both; }
  .sect-settle .st-sub{ animation:stFadeUp .46s ease-out .22s both; }
  .sect-settle .st-ledger{ transform-origin:top center; animation:stUnroll .42s cubic-bezier(.2,.8,.3,1) .30s both; }
  .sect-settle .st-row{ animation:stRowIn .34s ease-out both; }
  .sect-settle .st-v{ animation:stNumIn .28s ease-out both; }
  .sect-settle .st-row:nth-child(1){ animation-delay:.42s } .sect-settle .st-row:nth-child(1) .st-v{ animation-delay:.50s }
  .sect-settle .st-row:nth-child(2){ animation-delay:.50s } .sect-settle .st-row:nth-child(2) .st-v{ animation-delay:.58s }
  .sect-settle .st-row:nth-child(3){ animation-delay:.58s } .sect-settle .st-row:nth-child(3) .st-v{ animation-delay:.66s }
  .sect-settle .st-row:nth-child(4){ animation-delay:.66s } .sect-settle .st-row:nth-child(4) .st-v{ animation-delay:.74s }
  .sect-settle .st-row:nth-child(5){ animation-delay:.74s } .sect-settle .st-row:nth-child(5) .st-v{ animation-delay:.82s }
  .sect-settle .st-row:nth-child(n+6){ animation-delay:.82s } .sect-settle .st-row:nth-child(n+6) .st-v{ animation-delay:.90s }
  .sect-settle .st-notes{ animation:stFadeUp .42s ease-out .80s both; }
  .sect-settle .st-backlog{ animation:stFadeUp .42s ease-out .84s both; }
  .sect-settle .sect-btn.primary{ animation:stFadeUp .44s ease-out .88s both; }

  /* 结局：墨滴落纸、由浓渐淡地晕开，再托出评级与诗句 */
  .sect-ending{ text-align:center; max-width:680px; margin:30px auto; position:relative; padding:26px 18px; }
  .sect-ending::before{
    content:''; position:absolute; inset:0; border-radius:22px; pointer-events:none;
    background:radial-gradient(60% 50% at 50% 12%, rgba(232,198,106,.16), transparent 62%),
               radial-gradient(90% 80% at 50% 55%, rgba(0,0,0,.34), transparent 70%);
    animation:sectInkSpread 1.5s cubic-bezier(.2,.7,.3,1) both;
  }
  .sect-ending > *{ position:relative; }
  @keyframes sectInkSpread{
    0%  { opacity:0; transform:scale(.55); filter:blur(14px); }
    60% { opacity:1; filter:blur(2px); }
    100%{ opacity:1; transform:scale(1); filter:blur(0); }
  }
  .sect-ending .grade{ font-size:60px; font-family:var(--kai); color:var(--gold); animation:sectRiseIn .8s ease .35s both; }
  .sect-ending .et{ animation:sectRiseIn .8s ease .55s both; }
  .sect-ending .ed{ animation:sectRiseIn .8s ease .75s both; }
  .sect-ending .ep{ animation:sectRiseIn .8s ease 1.05s both; }
  @keyframes sectRiseIn{ 0%{ opacity:0; transform:translateY(10px); } 100%{ opacity:1; transform:none; } }
  .sect-ending .grade.D{ color:#e0795f; }
  .sect-ending .grade.S{ color:#e8c66a; text-shadow:0 0 20px rgba(232,198,106,.6); }
  .sect-ending .et{ font-size:26px; font-family:var(--kai); color:#f3e8c8; margin:8px 0; }
  .sect-ending .ed{ font-size:15px; color:#cabf9f; line-height:1.8; }
  .sect-ending .ep{ font-size:14px; color:#9c8f70; margin-top:14px; font-style:italic; }
  .sect-words{ display:flex; gap:8px; flex-wrap:wrap; margin-top:6px; }
  .sect-word{ font-size:11px; padding:2px 8px; border-radius:9px; border:1px solid rgba(232,198,106,.4); color:#e8cf8f; background:rgba(232,198,106,.08); }

  /* 因果 · 道契面板 */
  .sect-daoqi{ margin-bottom:14px; }
  .sect-daoqi .dq-st{ display:flex; gap:16px; flex-wrap:wrap; font-size:13px; color:#cdb98a; margin-bottom:6px; }
  .sect-daoqi .dq-st b{ font-style:normal; margin-left:4px; font-weight:600; }
  .sect-daoqi .dq-rel{ font-size:12px; color:#b9ad8f; }
  .sect-daoqi .dq-n{ cursor:help; border-bottom:1px dotted rgba(232,198,106,.35); }
  /* 人物志 */
  .sect-figs .fig-bar{ height:4px; background:rgba(255,255,255,.08); border-radius:3px; overflow:hidden; margin:6px 0 10px; }
  .sect-figs .fig-bar i{ display:block; height:100%; background:linear-gradient(90deg,#c9a227,#e8cf8f); transition:width .6s ease; }
  .sect-figs .fig-grid{ display:grid; grid-template-columns:repeat(auto-fill,minmax(152px,1fr)); gap:8px; }
  .fig-card{ background:linear-gradient(180deg,rgba(36,30,22,.66),rgba(20,17,15,.66)); border:1px solid rgba(232,198,106,.22); border-radius:10px; padding:8px 10px; }
  .fig-card summary{ cursor:pointer; list-style:none; display:flex; flex-direction:column; gap:2px; }
  .fig-card summary::-webkit-details-marker{ display:none; }
  .fig-card .fig-n{ font-family:var(--kai); font-size:15px; color:#f3e8c8; letter-spacing:1px; }
  .fig-card .fig-t{ font-size:11px; color:#a89b78; line-height:1.5; }
  .fig-card[open]{ border-color:rgba(232,198,106,.5); box-shadow:0 0 12px rgba(232,198,106,.12); }
  .fig-card[open] .fig-d, .fig-card[open] .fig-q{ animation:sectRiseIn .35s ease both; }
  .fig-card .fig-f{ display:inline-block; margin-top:8px; font-size:10px; padding:1px 7px; border-radius:8px; background:rgba(232,198,106,.14); color:#c9b98d; }
  .fig-card .fig-d{ margin-top:7px; font-size:12px; line-height:1.75; color:#cabf9f; }
  .fig-card .fig-q{ margin-top:7px; font-size:12px; color:#e8cf8f; font-style:italic; font-family:var(--kai); line-height:1.7; }
  .fig-card.lock{ opacity:.4; border-style:dashed; display:flex; align-items:center; justify-content:center; min-height:54px; }
  .fig-card.lock .fig-n{ font-size:20px; color:#7d7360; }
  .fig-card.lock .fig-t{ font-size:10px; }
  .ev-抉择{ background:rgba(232,198,106,.2); color:#f0d98a; border:1px solid rgba(232,198,106,.5); }

  /* ===================== 演出层：回合横幅 + 事件飘字 ===================== */
  /* 挂在 body 上、独立于 #app，重绘 innerHTML 不会清除，靠动画自销毁 */
  #sect-fx{ position:fixed; inset:0; pointer-events:none; z-index:60; overflow:hidden; }
  .sect-fx-banner{ position:absolute; left:50%; top:32%; transform:translate(-50%,-50%); text-align:center; opacity:0; }
  .sect-fx-banner.in{ animation:sectBannerIn 1.85s ease forwards; }
  .sect-fx-banner .bt{ font-family:var(--kai); font-size:42px; color:var(--gold); letter-spacing:10px;
    text-shadow:0 2px 20px rgba(0,0,0,.75), 0 0 28px rgba(232,198,106,.5); }
  .sect-fx-banner .bs{ font-size:15px; color:#e8dec4; letter-spacing:4px; margin-top:8px; }
  .sect-fx-banner .bline{ width:0; height:2px; margin:12px auto 0;
    background:linear-gradient(90deg,transparent,var(--gold),transparent); animation:sectLine 1.85s ease forwards; }
  .sect-fx-banner.danger .bt{ color:#e0795f; text-shadow:0 2px 20px rgba(0,0,0,.8), 0 0 28px rgba(224,121,95,.5); }
  .sect-fx-banner.danger .bline{ background:linear-gradient(90deg,transparent,#e0795f,transparent); }
  .sect-fx-banner.glory .bt{ color:#f0d98a; text-shadow:0 0 30px rgba(232,198,106,.9), 0 2px 20px rgba(0,0,0,.7); font-size:48px; }
  @keyframes sectBannerIn{
    0%{ opacity:0; transform:translate(-50%,-50%) scale(.82); letter-spacing:2px; filter:blur(3px); }
    18%{ opacity:1; transform:translate(-50%,-50%) scale(1); letter-spacing:10px; filter:blur(0); }
    78%{ opacity:1; }
    100%{ opacity:0; transform:translate(-50%,-52%) scale(1.06); }
  }
  @keyframes sectLine{ 0%{width:0;opacity:1} 28%{width:62%} 80%{width:62%} 100%{width:0;opacity:0} }

  /* 飘字：资源增减 / 收徒等，浮起淡出 */
  .sect-fx-float{ position:absolute; font-family:var(--kai); font-size:22px; font-weight:bold;
    white-space:nowrap; opacity:0; transform:translate(-50%,0); text-shadow:0 2px 10px rgba(0,0,0,.85);
    animation:sectFloatUp 1.3s ease-out forwards; }
  .sect-fx-float.good{ color:#7fc79a; }
  .sect-fx-float.bad{ color:#e0795f; }
  .sect-fx-float.neutral{ color:var(--gold); }
  @keyframes sectFloatUp{
  ...
  .sect-panel{ animation:sectInkIn .42s ease both; }
  .sect-ev{ animation:sectInkIn .5s ease both; }
  .sect-ev:nth-child(2){ animation-delay:.06s; }
  .sect-ev:nth-child(3){ animation-delay:.12s; }
  .sect-ev:nth-child(4){ animation-delay:.18s; }
  .sect-ev:nth-child(n+5){ animation-delay:.22s; }
  /* 册首语：翻页化开 */
  @keyframes sectPageOpen{
    0%{ opacity:0; clip-path:inset(0 0 100% 0); filter:blur(4px); }
    100%{ opacity:1; clip-path:inset(0 0 0 0); filter:blur(0); }
  }
  .sect-prologue{ animation:sectPageOpen .7s cubic-bezier(.2,.7,.2,1) both; }
  /* 选项悬停：剑气扫光 + 抬升 */
  .sect-opt{ position:relative; overflow:hidden; transition:background .15s,border-color .15s,transform .12s; }
  .sect-opt::after{
    content:''; position:absolute; top:0; left:-60%; width:55%; height:100%;
    background:linear-gradient(105deg, transparent, rgba(232,198,106,.22), transparent);
    transform:skewX(-18deg); transition:left .5s ease; pointer-events:none;
  }
  .sect-opt:not(:disabled):hover::after{ left:120%; }
  .sect-opt:not(:disabled):hover{ transform:translateY(-2px); border-color:var(--gold); }
  .sect-opt:active{ transform:translateY(0) scale(.99); }
  /* 按钮按压 */
  .sect-btn{ position:relative; overflow:hidden; transition:background .15s,transform .1s; }
  .sect-btn:active{ transform:translateY(1px) scale(.98); }
  /* 标题剑意流光 */
  .sect-title{ position:relative; }
  .sect-title::after{
    content:''; display:block; width:100%; height:2px; margin-top:6px;
    background:linear-gradient(90deg, transparent, var(--gold), transparent);
    background-size:200% 100%; animation:sectTitleFlow 4.5s linear infinite;
  }
  @keyframes sectTitleFlow{ 0%{background-position:200% 0} 100%{background-position:-200% 0} }
  .sect-panel h3{ transition:color .2s, letter-spacing .2s; }
  .sect-panel h3:hover{ color:#f3d887; letter-spacing:3px; }
  /* 紧迫事件呼吸光 */
  @keyframes sectUrgentPulse{
    0%,100%{ box-shadow:0 0 0 rgba(224,121,95,0); }
    50%{ box-shadow:0 0 16px rgba(224,121,95,.35); }
  }
  .sect-ev.urgent{ animation:sectInkIn .5s ease both, sectUrgentPulse 2.4s ease-in-out .5s infinite; }
  /* 节点横幅墨溅 */
  .sect-fx-banner::before{
    content:''; position:absolute; left:50%; top:50%;
    border-radius:50%;
    background:radial-gradient(circle, rgba(232,198,106,.55), transparent 70%);
    transform:translate(-50%,-50%); animation:sectSplash 1.85s ease forwards;
  }
  @keyframes sectSplash{ 0%{ width:6px;height:6px;opacity:.9 } 40%{ width:340px;height:340px;opacity:.25 } 100%{ width:520px;height:520px;opacity:0 } }
  /* 点击墨溅（由 sectWireFx 注入 body） */
  .sect-fx-ripple{
    position:fixed; border-radius:50%; pointer-events:none; z-index:9999;
    border:2px solid rgba(232,198,106,.7);
    transform:translate(-50%,-50%) scale(0); animation:sectRipple .6s ease-out forwards;
  }
  @keyframes sectRipple{ 0%{ transform:translate(-50%,-50%) scale(0); opacity:.8 } 100%{ transform:translate(-50%,-50%) scale(1); opacity:0 } }
  /* 结局 S 剑气横扫 */
  .sect-ending .grade.S{ animation:sectSGlow 2.6s ease-in-out infinite; }
  @keyframes sectSGlow{ 0%,100%{ text-shadow:0 0 20px rgba(232,198,106,.6); } 50%{ text-shadow:0 0 38px rgba(232,198,106,.95); } }
  .sect-fx-float{ text-shadow:0 2px 10px rgba(0,0,0,.6); }

  /* —— 沉浸化：叙事节拍 + 山务抽屉（渐进式呈现，避免一屏铺满） —— */
  .sect-mood{ font-size:13.5px; color:#b9ad8f; font-style:italic; padding:4px 2px 11px; border-bottom:1px dashed rgba(232,198,106,.12); margin-bottom:12px; line-height:1.75; }
  .sect-mood::before{ content:'「'; color:var(--gold); opacity:.5; margin-right:1px; }
  .sect-mood::after{ content:'」'; color:var(--gold); opacity:.5; margin-left:1px; }
  .sect-stage{ box-shadow:0 0 0 1px rgba(232,198,106,.2); }
  .sect-actions .sect-btn[onclick^="toggleShanwu"]{ border-color:var(--gold); background:rgba(232,198,106,.16); }
  .sect-drawer-mask{ position:fixed; inset:0; background:rgba(8,6,4,.55); z-index:40; animation:sectFade .22s ease both; }
  @keyframes sectFade{ from{opacity:0} to{opacity:1} }
  .sect-drawer{ position:fixed; top:0; right:0; height:100%; width:min(460px,94vw); z-index:41; background:linear-gradient(180deg,rgba(28,22,16,.985),rgba(18,15,11,.985)); border-left:1px solid rgba(232,198,106,.35); box-shadow:-14px 0 44px rgba(0,0,0,.55); overflow-y:auto; padding:14px 16px 36px; animation:sectDrawerIn .3s cubic-bezier(.2,.7,.2,1) both; }
  @keyframes sectDrawerIn{ from{transform:translateX(100%)} to{transform:translateX(0)} }
  .sect-drawer-h{ display:flex; align-items:center; justify-content:space-between; font-family:var(--kai); font-size:19px; color:var(--gold); letter-spacing:3px; margin-bottom:12px; padding-bottom:8px; border-bottom:1px solid rgba(232,198,106,.3); }
  .sect-mg-actions{ margin:0 0 14px; }
  .sect-drawer .sect-grid{ grid-template-columns:1fr; }
  .sect-drawer .sect-figs .fig-grid{ grid-template-columns:repeat(auto-fill,minmax(140px,1fr)); }
  /* 山志·原著脉络：竖向时间轴，原著向、不堆主舞台 */
  .sect-chron{ position:relative; padding-left:4px; }
  .sect-chron-i{ position:relative; padding:0 0 18px 18px; border-left:1px solid rgba(232,198,106,.26); }
  .sect-chron-i:last-child{ border-left-color:transparent; }
  .sect-chron-dot{ position:absolute; left:-6px; top:5px; width:11px; height:11px; border-radius:50%; background:var(--gold); box-shadow:0 0 8px rgba(232,198,106,.6); }
  .sect-chron-t{ font-family:var(--kai); font-size:16px; color:var(--gold); letter-spacing:1px; }
  .sect-chron-d{ font-size:13.5px; color:#cabf9f; line-height:1.7; margin-top:4px; }
  /* 山志原文引述：楷体、左侧金线，与正文区隔，标明这是原著文字 */
  .sect-chron-q{ font-family:var(--kai); font-size:13px; color:#d8c68d; line-height:1.8;
                 margin:7px 0 0; padding:5px 0 5px 11px; border-left:2px solid rgba(232,198,106,.42); }
  .sect-chron-src{ font-size:11px; color:#8a8170; margin-top:5px; }
  `;
...
/* ===================== 演出层（横幅 / 飘字，独立于 #app） ===================== */
function sectFXInit(){
  if(document.getElementById('sect-fx')) return;
  const d = document.createElement('div');
  d.id = 'sect-fx';
  document.body.appendChild(d);
}
function sectFXClear(){
  const fx = document.getElementById('sect-fx');
  if(fx) fx.innerHTML = '';
  sectBannerQueue = [];
  sectBannerBusy = false;
}
let sectBannerQueue = [];
...
function sectBannerShow(){
  if(!sectBannerQueue.length){ sectBannerBusy = false; return; }
  sectBannerBusy = true;
  const it = sectBannerQueue.shift();
  const fx = document.getElementById('sect-fx');
  const el = document.createElement('div');
  el.className = 'sect-fx-banner in' + (it.cls ? ' '+it.cls : '');
  el.innerHTML = '<div class="bt">'+it.title+'</div>'
    + (it.sub ? '<div class="bs">'+it.sub+'</div>' : '')
    + '<div class="bline"></div>';
  fx.appendChild(el);
  setTimeout(function(){ el.remove(); sectBannerShow(); }, 1880);
}
function sectFloat(text, cls, x, y){
  sectFXInit();
  const fx = document.getElementById('sect-fx');
  const el = document.createElement('div');
  el.className = 'sect-fx-float ' + (cls||'neutral');
  el.textContent = text;
  el.style.left = (x!=null ? x : (window.innerWidth/2)) + 'px';
  el.style.top  = (y!=null ? y : (window.innerHeight*0.4)) + 'px';
  fx.appendChild(el);
  setTimeout(function(){ el.remove(); }, 1320);
}
/* 资源增减飘字：在资源栏上方散开浮起 */
function sectSpawnFx(s){
  if(!s.fxQueue || !s.fxQueue.length) return;
  const app = document.getElementById('app');
  let baseX = window.innerWidth/2, baseY = window.innerHeight*0.28;
  if(app){
    const res = app.querySelector('.sect-res');
    if(res){ const r = res.getBoundingClientRect(); baseX = r.left + r.width/2; baseY = r.top + r.height/2; }
  }
  const n = s.fxQueue.length;
  s.fxQueue.forEach(function(f, i){
    let txt, cls;
    if(f.v === 0){ txt = f.k; cls = 'neutral'; }
    else { const sign = f.v>0 ? '+' : ''; txt = f.k + ' ' + sign + f.v; cls = f.v>0 ? 'good' : 'bad'; }
    const x = baseX + (i - (n-1)/2) * 66 + (Math.random()*18 - 9);
    const y = baseY - i*6 + (Math.random()*12 - 6);
    sectFloat(txt, cls, x, y);
  });
  s.fxQueue = [];
}

/* 点击墨溅：不阻塞原有 onclick，仅作视觉反馈（捕获阶段注入 body，随重绘不丢失） */
let sectFxWired = false;
function sectWireFx(){
  if(sectFxWired) return; sectFxWired = true;
  if(typeof document==='undefined' || !document.addEventListener || !document.body) return;
  document.addEventListener('click', function(e){
    const t = e.target && e.target.closest ? e.target.closest('.sect-opt, .sect-btn, .sect-card-row') : null;
    if(!t) return;
    const x = e.clientX, y = e.clientY;
    if(x==null || y==null) return;
    const r = document.createElement('div');
    r.className = 'sect-fx-ripple';
    const size = Math.max(t.offsetWidth, t.offsetHeight) * 2.2;
...
/* 弟子：base=每旬修为成长，stage 0学徒/1中境/2大成
   rank='弟子' 为陈平安门下八人（据百度百科「落魄山」词条：崔东山、裴钱、曹晴朗、赵树下、
   郭竹酒、宁吉、邓剑枰、袁黄，收徒重品性轻天赋）；rank='门中' 为山中常住而非入室弟子者。
   李宝瓶是齐静春弟子（称陈平安「小师叔」）、顾璨是泥瓶巷兄弟，皆非陈平安门下，不入此表。 */
const SECT_DISCIPLES = {
  peiqian:  { name:'裴钱', art:'裴钱', dao:'剑',  realm:'练气', base:3, loyal:70, rank:'弟子',
    desc:'藕花福地南苑国京城外粥铺，那个枯瘦黝黑、揣一把豁口刀的野丫头。出福地后一路跟着不走，是落魄山开山大弟子。性子野、最爱银钱、极护短，一句「师父」叫到末尾。' },
...
/* 门中 · 供奉 · 管家：不入弟子籍，占「门中位」，按旬支俸，换战力与名望。
   据百度百科「落魄山」：大管家朱敛，小管家陈如初（暖树）；首席供奉姜尚真；
   一等供奉小陌、谢狗（化名白景）、天然、种秋、米裕、卢白象、隋右边、魏羡、陈灵均、石柔；
   掌律长命，账房韦文龙（泉府掌舵）。魏檗为棋墩山土地/披云山山神，落魄山「住山大使」。 */
const SECT_RETINUE = {
  ...
  xiaomo:   { name:'小陌', art:'小陌', rank:'一等供奉', pow:30, pay:5, 名:5,
    desc:'飞升境巅峰剑修，本在皓彩明月里沉睡万年，被持剑者带来给你当保镖。话极少，剑极快。' },
  xiegou:   { name:'谢狗', art:'谢狗', rank:'一等供奉', pow:28, pay:5, 名:4,
    desc:'化名白景的远古大妖，飞升境巅峰圆满剑修，两把本命飞剑。她来落魄山，是因为小陌在这儿。' },
...
/* 人情债：夜游宴一类「借来的体面」记在此处。债不消，迟早有人上门。 */
...
const SECT_EVENTS = [
  ...
  // —— 补原著：于桐叶洲择址建落魄山下宗——青萍剑宗；又开凿大渎勾连桐叶洲与东海水域。
  ...
  { id:'qingping', chapter:6, bookXun:54, type:'抉择', urgent:true, once:'qingping', title:'青萍剑宗 · 下宗',
    desc:'上宗落魄山既立，你要在桐叶洲再开一座下宗。宗门叫什么、立什么规矩，先得想清楚。「青萍」二字，是那柄剑的名字，也是你这一路走过来的意思。',
    opts:[
...
function newSectState(diffKey){
  ...
  return {
    ...
    ui:'main',
    shanwu:false,           // 山务抽屉默认收起（渐进式呈现，点击「山务」才展开）
    chronicle:false,         // 山志·原著脉络抽屉：默认收起，点击「山志」才展开
    log: [],
    fxQueue: [],
    ...
    settling:false,                      // 本旬结算中转页标记
    lastSettle:null,                     // 最近一次结算快照（账本）
  };
}
...
function openSect(){
  ...
  state = { phase:'sect', sect:newSectState(difficulty) };
  const s = state.sect;
  sectLog(s, '你于骊珠洞天降格之际，立落魄山。山主之责，自此而始。', 'big');
  // 开局即展示第一册册首语（章内"节"的仪式感）
  const first = sectCurrentBook(s);
  s.bookKey = first.book;
  s.prologue = { title:'《'+first.book+'》', sub:'第 '+first.act+' 幕 · '+first.actName+'　第 '+first.bookIdxInAct+' / '+first.bookCountInAct+' 册', body: SECT_BOOK_PROLOGUE[first.book] || '' };
...
/* 【不剧透纪律】此处原有一支 sectOptGainText()，把「银+5 / 道心-3」这类收益
   算成预览贴在选项上。已随 2026-08-31 的交互纠偏一并删除（与小程序端
   utils/sect.js 对齐）：玩家只需要做决断，后果在提交后由 sectApplyEff 推入
   s.fxQueue 变成飘字，并写入 sectTrail 山中纪事后揭示。
   若将来要在别处展示收益，请先确认是否属于「事前剧透」，不要直接塞回选项按钮。 */
...
function sectResolveEvent(idx, optIdx){
  const s = state.sect; if(s.over) return;
...
function sectBuild(key){
  ...
  if(!cost){ return; }
  if(!sectSpendRes(s, cost)){ sectLog(s,'银/灵/名不足，建不成「'+def.name+'」。','bad'); render(); return; }
  if(!sectUseAp(1)){ sectLog(s,'本旬行动点已尽，无暇营建。','bad'); render(); return; }
  b.lv++;
...
function sectRecruitSeek(){
  const s = state.sect; if(s.over) return;
  if(!sectUseAp(1)){ sectLog(s,'本旬行动点已尽。','bad'); render(); return; }
  // 从尚未收入、且条件可收的弟子里随机给一个「投靠」机会
  const pool = Object.keys(SECT_DISCIPLES).filter(k=>!s.disciples.some(d=>d.key===k));
  if(!pool.length){ sectLog(s,'门下已无可知的晚辈可寻。',''); render(); return; }
...
function sectForge(){
  ...
  if(!sectUseAp(1)){ sectLog(s,'本旬行动点已尽。','bad'); render(); return; }
  const cost = { 灵:10, 银:5 };
  if(!sectSpendRes(s, cost)){ sectLog(s,'灵机或银两不足，无从祭炼。','bad'); render(); return; }
...
function sectBreak(){
  ...
  if(cur>=SECT_REALMS.length-1){ sectLog(s,'你已至当世绝顶，无可再破。',''); render(); return; }
  const need = SECT_MASTER_CULT[cur+1];
  if(s.masterCult < need){ sectLog(s,'修为未足（山主修为 '+s.masterCult+'/'+need+'），破境无门。','bad'); render(); return; }
  s.breakPending = need;
...
function sectGamble(big){
  ...
  if(bieyuan<1){ sectLog(s,'须先建倒悬山别院，方能开赌。','bad'); render(); return; }
  if(!sectUseAp(1)){ sectLog(s,'本旬行动点已尽。','bad'); render(); return; }
  const cost = big?25:10;
  if(s.res.银 < cost){ sectLog(s,'银两不足，押不进这局。','bad'); render(); return; }
  sectGain(s,'银', -cost);
...
/* 叙事节拍：每步一句氛围引子，按「幕 + 季节 + 是否待决」轮替，绝不剧透本旬选项。
   营造「逐步推进」的沉浸感（沉浸化范式：叙事为主、系统为暗线）。 */
function sectMoodLine(s){
  const b = sectCurrentBook(s);
  const actLines = {
    1:['骊珠洞天的风，还带着少年时的味道。','泥瓶巷的雨，落了又停。','旧瓷碎片里，藏着一个少年最开始的命。'],
    2:['草长莺飞，新山初立，万事待兴。','山门前的松，才栽下不久。','远处有客来，脚步尚轻。'],
    3:['老龙城外，书简湖上，因果如网。','风过处，皆是旧人的名姓。','一盏灯下，算不清的账。'],
    4:['剑气长城的雪，落在每个守城人的肩头。','城头风急，剑鸣如诉。','有人在等，也有人已经走了。'],
    5:['蛮荒的风，自远方来了。','枕戈待旦，这一夜格外长。','该守的，不止一座山。'],
    6:['天门将开，这一生，该有个交代。','山有山河，心有秤。','最后一问，是问自己。'],
  };
  let pool = actLines[b.act] || actLines[6];
  const se = sectSeason(s);
  if(se==='冬') pool = ['山中冬日，炉火尚可温酒。','雪压松枝，山门寂寂。'].concat(pool);
  else if(se==='秋') pool = ['秋深了，山里多添了几分肃杀。'].concat(pool);
  if(s.events.some(e=>{ const d=sectEventDef(e.id); return d && d.urgent; })) pool = ['案头有信，须你拿主意。'].concat(pool);
  return pool[s.xun % pool.length];
}

/* ===================== 全册脊（真实册目·六幕） ===================== */
// 真实册目：浙江文艺出版社实体书最终 8 辑 54 册（2025-10 出齐）。
// 6 幕按原著主线阶段切分，覆盖全部 54 个真实四字册名，无编造、无遗漏、顺序不乱。
// 幕界依原著锚点：册22《愿挽天倾者》陈平安斩离真、接任隐官；册23《人生梦复梦》剑气长城飞升、陈平安独守；
//                 册27《风雪夜归人》崔瀺身死道消、陈平安回落魄山创建宗门；册28《清都山水郎》祖师堂观礼、跻身「宗」字头。
const SECT_ACTS = [
...
// 山志 · 原著脉络：自泥瓶巷到万山朝奉，严格按《剑来》原著时序排列。
// 一处最易搞混的地方，也是本作玩法的立身之本——
//   落魄山「买山」极早：骊珠洞天坠落后，陈平安在阮邛帮助下以金精铜钱买下五座山头，落魄山其一。
//   落魄山「开宗」极晚：第27册《风雪夜归人》创建宗门，第28册《清都山水郎》祖师堂观礼、跻身「宗」字头。
// 中间二十余册，山主常年在外行路（负笈、送剑、问心、守城），山中事务交由崔东山、朱敛、陈灵均、陈暖树等人打理。
// 玩家所代者，正是这一份「山主不在山中、而山不可一日无主」的守山之责。
...
function sectSettle(){
  const s = state.sect; if(s.over || s.settling) return;
...
function sectEnterNext(){
  ...
  if(s.bookKey !== cur.book){
    s.bookKey = cur.book;
    s.prologue = { title:'《'+cur.book+'》', sub:'第 '+cur.act+' 幕 · '+cur.actName+'　第 '+cur.bookIdxInAct+' / '+cur.bookCountInAct+' 册', body: SECT_BOOK_PROLOGUE[cur.book] || '' };
  }
  ...
  const bieyuan = s.buildings['倒悬山别院']?s.buildings['倒悬山别院'].lv:0;
  for(let i=0;i<n;i++){
    let pool = SECT_EVENTS.filter(e=>!e.crisis && !e.chapter && (!e.gate || e.gate(s)) && (!e.once || !s.flags[e.once]) && s.nodeFired['qulu']);
    if(bieyuan>0){
...
/* 节点：按原著时序排列，与 SECT_ACTS 六幕一一对应（册号＝浙江文艺出版社实体书）。
   qulu 骊珠降格(册4) → dabi 山崖书院(册16) → shujian 书简湖问心局(册21)
   → qiguan 剑气长城倾覆(册25) → kaizong 落魄开宗(册28) → manhuang 蛮荒叩关(册46) → ascend 建宗大典(册54) */
const SECT_NODES = {
...
function sectSaveAllies(s){
  try{
    ...
    localStorage.setItem('jianlai_sect_allies', JSON.stringify(best));
  }catch(e2){}
}
...
function sectEnding(s, key){
  ...
  // 行迹录：经营终局计入道行与见闻
  try{ if(typeof prRecordEnding==='function') prRecordEnding('sect', key, e.t, isNew); }catch(e2){}
...
/* 落魄山资源条器物图标：单色描边、stroke-width 2、round 端点/转角、无渐变光晕。
   颜色用 currentColor，由 .sect-res-i .ico 的 color 控制（金）。viewBox 0 0 30 30。 */
const SECT_ICO = {
  yinliang: '<svg viewBox="0 0 30 30" width="24" height="24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">'
    + '<circle cx="11" cy="15" r="7" fill="none" stroke="currentColor" stroke-width="2"/>'
    + '<rect x="8.5" y="12.5" width="5" height="5" fill="none" stroke="currentColor" stroke-width="1.4"/>'
    + '<circle cx="22" cy="19" r="6" fill="none" stroke="currentColor" stroke-width="2"/>'
    + '<rect x="19.5" y="16.5" width="5" height="5" fill="none" stroke="currentColor" stroke-width="1.4"/>'
    + '<path d="M11 8 Q16 4 22 13" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round"/></svg>',
  lingji: '<svg viewBox="0 0 30 30" width="24" height="24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">'
    + '<path d="M6 20 Q6 14 12 14 Q14 9 19 12 Q24 10 24 16 Q28 16 26 20 Z" fill="none" stroke="currentColor" stroke-width="2" stroke-linejoin="round" stroke-linecap="round"/>'
    + '<path d="M9 24 Q9 21 13 21 Q15 18 19 20" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-opacity=".6"/></svg>',
  mingsheng: '<svg viewBox="0 0 30 30" width="24" height="24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">'
    + '<path d="M10 7 L20 7 L19 9 L19 25 L11 25 L11 9 Z" fill="none" stroke="currentColor" stroke-width="2" stroke-linejoin="round" stroke-linecap="round"/>'
    + '<path d="M14 12 L16 12 M14 16 L16 16 M14 20 L16 20" stroke="currentColor" stroke-width="1.6" stroke-linecap="round"/>'
    + '<path d="M9 25 L21 25" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"/></svg>',
  daoxin: '<svg viewBox="0 0 30 30" width="24" height="24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">'
    + '<path d="M11 22 L19 22 L17 26 L13 26 Z" fill="none" stroke="currentColor" stroke-width="2" stroke-linejoin="round" stroke-linecap="round"/>'
    + '<path d="M15 22 L15 18" stroke="currentColor" stroke-width="1.6" stroke-linecap="round"/>'
    + '<path d="M15 18 Q11 14 15 10 Q19 14 15 18 Z" fill="none" stroke="currentColor" stroke-width="2" stroke-linejoin="round" stroke-linecap="round"/></svg>',
  jiyuan: '<svg viewBox="0 0 30 30" width="24" height="24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">'
    + '<path d="M11 12 L11 26 L19 26 L19 12 Z" fill="none" stroke="currentColor" stroke-width="2" stroke-linejoin="round" stroke-linecap="round"/>'
    + '<path d="M11 12 Q15 9 19 12" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"/>'
    + '<line x1="13" y1="8" x2="13" y2="14" stroke="currentColor" stroke-width="1.6" stroke-linecap="round"/>'
    + '<line x1="17" y1="8" x2="17" y2="14" stroke="currentColor" stroke-width="1.6" stroke-linecap="round"/>'
    + '<path d="M12 20 L18 20 M12 23 L18 23" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-opacity=".55"/></svg>',
  dan: '<svg viewBox="0 0 30 30" width="24" height="24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">'
    + '<path d="M13 9 L13 12 Q9 13 9 17 Q9 21 13 21 Q13 24 15 24 Q17 24 17 21 Q21 21 21 17 Q21 13 17 12 L17 9 Z" fill="none" stroke="currentColor" stroke-width="2" stroke-linejoin="round" stroke-linecap="round"/>'
    + '<path d="M13 9 L13 7 M17 9 L17 7" stroke="currentColor" stroke-width="1.6" stroke-linecap="round"/>'
    + '<circle cx="13" cy="18" r="1.2" fill="currentColor" fill-opacity=".6"/></svg>',
};
function sectResIcon(k){
  return SECT_ICO[{银:'yinliang',灵:'lingji',名:'mingsheng',道心:'daoxin',机缘:'jiyuan',丹:'dan'}[k]] || '';
}
/* 事件选项图标：朴素古风小图。不上渐变/光晕。type 与 shenci 同源（访客/讲道理/天劫/危机/册目/奇遇/江湖） */
function sectOptIcon(type){
  if(type==='天劫' || type==='危机'){
  ...
  if(type==='讲道理'){
    return '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M4 5 H20 V17 H8 L4 21 Z"/><path d="M8 10 H16"/><path d="M8 13 H13"/></svg>';
  }
  if(type==='册目'){
  ...
  if(type==='奇遇'){
    return '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M12 3 L14 9 L20 10 L15.5 14.5 L17 21 L12 17.5 L7 21 L8.5 14.5 L4 10 L10 9 Z"/></svg>';
  }
  if(type==='访客'){
    return '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><circle cx="9" cy="8" r="3"/><path d="M3 20 a6 6 0 0 1 12 0"/><circle cx="17" cy="9" r="2"/><path d="M14 20 a4 4 0 0 1 7 0"/></svg>';
  }
  if(type==='江湖'){
    return '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M3 12 Q6 8 9 12 T15 12 T21 12"/><path d="M3 17 Q6 13 9 17 T15 17 T21 17"/></svg>';
  }
  // 默认（祈愿/事件/未明）：香炉
...
function sectRenderRes(s){
  const order = [['银','银两'],['灵','灵机'],['名','名声'],['道心','道心'],['机缘','机缘'],['丹','丹']];
  let h = '<div class="sect-res">';
  order.forEach(([k,lab])=>{
    h += '<div class="sect-res-i"><span class="ico">'+sectResIcon(k)+'</span>'
       + '<div class="k">'+lab+'</div>'
       + '<div class="v '+sectResClass(s,k)+'">'+s.res[k]+'</div></div>';
  });
  h += '</div>';
  return h;
}
function sectRenderNodes(s){
  let h = '<div class="sect-nodes">';
  SECT_NODE_ORDER.forEach(k=>{
    const nd = SECT_NODES[k];
    const total = 60;
    const prog = Math.min(100, Math.round((s.xun/nd.xun)*100));
    const cls = s.nodeFired[k] ? 'done' : (s.xun>=nd.xun-3 ? 'fire' : '');
    h += '<span class="sect-node '+cls+'">'+nd.name
       + '<span class="bar"><i style="width:'+prog+'%"></i></span></span>';
  });
  h += '</div>';
  return h;
}
function sectClosePrologue(){
  const s = state.sect; if(!s || !s.prologue) return;
  s.prologue = null; render();
}
function sectRenderEvents(s){
  if(!s.events.length) return '<div class="sect-empty">本旬无事，可安心经营，或主动寻访、祭炼。</div>';
  let h = '<div class="sect-events">';
  s.events.forEach((ev,idx)=>{
    const def = sectEventDef(ev.id); if(!def) return;
    const urg = def.urgent ? ' urgent' : '';
    const ask = def.type==='问心' ? ' ask' : '';
    h += '<div class="sect-ev'+urg+ask+'">';
    h += '<span class="sect-ev-type ev-'+def.type+'">'+def.type+'</span>';
    h += '<h4>'+def.title+'</h4>';
    h += '<p>'+def.desc.replace(/\n/g,'<br>')+'</p>';
    if(def.q) h += '<blockquote class="sect-quote">「'+def.q.replace(/</g,'&lt;')+'」</blockquote>';
    /* 选项只呈现「你要怎么做」本身。
       收益预览（gain：银+5 之类）与旁白解读（note）一律不事前剧透，选完之后
       由 fxQueue 飘字（银+5 / 道心-3）+ sectTrail 山中纪事揭示。
       条件不满足时的锁定原因（req）属约束提示，保留。 */
    def.opts.forEach((opt,oi)=>{
      const rq = sectOptReqOk(s, opt);
      h += '<button class="sect-opt" '+(rq.ok?'onclick="sectResolveEvent('+idx+','+oi+')"':'disabled')+'>'
         + '<span class="sect-opt-ico">'+sectOptIcon(def.type)+'</span>'
         + '<span>'+opt.txt+'</span>'
         + (rq.ok?'':'<span class="req">'+rq.why+'</span>')
         + '</button>';
    });
    h += '<button class="sect-opt skip-opt" onclick="sectSkipEvent('+idx+')">'
       + '<span class="sect-opt-ico"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M5 19 H19"/><path d="M8 19 V12 H16 V19"/><path d="M8 12 H16"/><path d="M12 5 V9"/><path d="M9.5 7.5 L12 5 L14.5 7.5"/></svg></span>'
       + '<span>搁置（占积压）</span></button>';
    h += '</div>';
  });
  h += '</div>';
  return h;
}
function sectRenderBuildings(s){
  let h = '';
  for(const k in SECT_BUILDINGS){
    ...
    h += '<div class="sect-card-row">'
       + '<div><div class="sect-card-name">'+def.name+' <span class="sect-lv">Lv.'+b.lv+'/'+def.max+'</span></div>'
       + '<div class="sect-card-sub">'+(def.eff[b.lv]||def.desc)+'</div></div>'
       + (maxed? '<span class="sect-lv">满</span>'
                 : '<button class="sect-btn sm" onclick="sectBuild(\''+k+'\')">升 · '+costTxt+'</button>')
       + '</div>';
  }
  return h;
}
function sectRenderDisciples(s){
  if(!s.disciples.length) return '<div class="sect-empty">门下尚无弟子。可于事件中收徒，或主动寻访。</div>';
  let h = '';
  s.disciples.forEach(d=>{
    const loyalCls = d.loyal<25?'warn':(d.loyal>70?'good':'');
    const art = d.art ? '<img class="sect-portrait" src="art/'+d.art+'.png" alt="'+d.name+'" onerror="this.style.display=\'none\'">' : '';
    h += '<div class="sect-card-row sect-disc">'
       + art
       + '<div class="dinfo"><div class="sect-card-name">'+d.name+' <span class="sect-lv">'+SECT_REALMS[sectRealmOf(d.xiwei)]+'境 · '+d.dao+'道</span></div>'
       + '<div class="sect-card-sub">修为 '+d.xiwei+' · 忠心 <span class="'+loyalCls+'">'+d.loyal+'</span></div></div>'
       + '</div>';
  });
  return h;
}
function sectRenderSwords(s){
  if(!s.swords.length) return '<div class="sect-empty">尚无飞剑。于剑庐祭炼，可得剑胚与本命飞剑。</div>';
  let h = '';
  s.swords.forEach(x=>{
    h += '<div class="sect-card-row">'
       + '<div><div class="sect-card-name">'+x.name+' <span class="sect-lv">阶'+x.tier+' · '+x.dao+'道</span></div>'
       + '<div class="sect-words">'+x.words.map(w=>'<span class="sect-word">'+w+'</span>').join('')+'</div></div>'
       + '</div>';
  });
  return h;
}
function sectRenderLog(s){
  let h = '<div class="sect-log">';
  s.log.forEach(l=>{ h += '<div class="lg '+l.cls+'">'+l.t+'</div>'; });
  h += '</div>';
  return h;
}

/* 因果 · 道契 面板：让玩家看见每一次选择累积出的「你成了怎样的人」 */
function sectRenderDaoqi(s){
  const axis = (label, v, pos, neg) => {
    const side = v>0 ? pos : v<0 ? neg : '未定';
    const col = v>0 ? '#e8a06a' : v<0 ? '#6ab0e8' : '#b9ad8f';
    return '<span>'+label+'：<b style="color:'+col+'">'+side+'</b></span>';
  };
  const relKeys = SECT_REL_ORDER.filter(k=> s.relations[k]);
  // 结缘者标出原著称号（悬停可见），道契数字之外多一层「此人是谁」
  const rel = relKeys.length ? relKeys.map(k=>{
    const f = SECT_FIGURES[k];
    const tip = f ? ' title="'+f.t+'"' : '';
    return '<span class="dq-n"'+tip+'>'+k+' '+(s.relations[k]>0?'+':'')+s.relations[k]+'</span>';
  }).join('　') : '';
  return '<div class="sect-panel sect-daoqi"><h3>因果 · 道契</h3>'
    + '<div class="dq-st">'
    +   axis('立身', s.stance['剑礼'], '以剑', '以礼')
    +   axis('所守', s.stance['守'], '守一人', '守天下')
    +   axis('行事', s.stance['势'], '顺势', '硬扛')
    + '</div>'
    + '<div class="dq-rel">'+(rel || '尚未结下深缘')+'</div></div>';
}
/* 人物志：按此局进度点亮已相遇的原著人物。
   纯展示判定（读 relations / flags / xun），不写状态、不触因果与结局。 */
function sectRenderFigures(s){
  const keys = Object.keys(SECT_FIGURES);
  const met  = keys.filter(k=>{ const u = SECT_FIGURES[k].unlock; return !u || u(s); });
  const pct  = Math.round(met.length / keys.length * 100);
  let h = '<div class="sect-panel sect-figs"><h3>人物志 · 已识 '+met.length+' / '+keys.length+'</h3>'
        + '<div class="fig-bar"><i style="width:'+pct+'%"></i></div><div class="fig-grid">';
  keys.forEach(k=>{
    const f = SECT_FIGURES[k];
    if(met.indexOf(k) < 0){
      h += '<div class="fig-card lock" title="尚未相遇"><div class="fig-n">？</div><div class="fig-t">未识</div></div>';
      return;
    }
    h += '<details class="fig-card"><summary>'
       +   '<span class="fig-n">'+k+'</span><span class="fig-t">'+f.t+'</span>'
       + '</summary>'
       + '<div class="fig-f">'+f.f+'</div>'
       + '<div class="fig-d">'+f.d+'</div>'
       + '<div class="fig-q">「'+f.q+'」</div>'
       + '</details>';
  });
  h += '</div></div>';
  return h;
...
function renderSect(app){
  ...
  if(s.over){
    ...
    let html = '<div class="sect"><div class="sect-ending">'
      + '<img class="sect-end-img" src="assets/sect/'+s.ending+'.jpg" alt="'+e.t+'" onerror="this.style.display=\'none\'">'
      + '<div class="grade '+e.g+'">'+e.g+'</div>'
      + '<div class="et">'+e.t+'</div>'
      + '<div class="ed">'+e.d.replace(/\n/g,'<br>')+'</div>'
      + '<div class="ep">「'+e.poem+'」</div>'
      + '<div class="sect-words" style="justify-content:center;margin-top:12px;max-width:560px;margin-left:auto;margin-right:auto;text-align:center;line-height:1.9">'+sectDaoqiRecap(s)+'</div>'
      + '<div class="sect-words" style="justify-content:center;margin-top:10px">门下弟子与山主境界已载入「剑来·择主」，可携入卡牌对战。</div>'
      + '<div class="sect-back">'
      + '<button class="sect-btn primary" onclick="openSect()">重 开 一 局</button> '
      + '<button class="sect-btn ghost" onclick="state={phase:\'title\',sel:[],log:[]};render()">回到卷首</button>'
      + '</div></div></div>';
    app.innerHTML = html; return;
  }

  // 本旬结算中转页：先出账本，确认后再入下一旬
  if(s.settling && s.lastSettle){
    const st = s.lastSettle;
    let lh = '';
    st.ledger.forEach(r=>{
      const cls = r.d>0?'up':(r.d<0?'down':'');
      const dTxt = (r.d>0?'+':'')+r.d;
      lh += '<div class="st-row '+cls+'"><div class="st-k">'+r.k+'</div><div class="st-v">'+dTxt+'</div></div>';
    });
    let nh = '';
    ...
    app.innerHTML = '<div class="sect"><div class="sect-settle">'
      ...
      + backlog
      + '<button class="sect-btn primary" onclick="sectEnterNext()">入 下 一 旬 →</button>'
      + '</div></div>';
    return;
  }
  ...
  html += '<div class="sect-head">'
        + '<div class="sect-hero">'
        +   '<img class="ph" src="art/chenpingan.png" alt="山主陈平安" onerror="this.style.display=\'none\'">'
        +   '<div class="phtxt"><div class="phname">落魄山</div>'
        +   '<div class="phsub">山主 · 陈平安　·　'+SECT_REALMS[s.masterRealm]+'境　·　第 '+s.year+' 年</div></div>'
        + '</div>'
        + '<div style="color:#e8cf8f;font-size:13px;margin-bottom:3px">第 '+book.act+' 幕 · '+book.actName+'　｜　第 '+book.bookIdxInAct+' / '+book.bookCountInAct+' 册《'+book.book+'》</div>'
        + '<div class="sect-timer">第 <b>'+((s.xun-1)%12+1)+'</b> 旬 · <span class="sect-season">'+sectSeason(s)+'</span> · 距建宗大典尚有 <b>'+(SECT_NODES.ascend.xun - s.xun)+'</b> 旬'
        + '<br><span style="color:#cdb98a">山主修为 '+s.masterCult+'　'+(s.masterRealm>=SECT_REALMS.length-1?'（已至绝顶）':'· 下境需 '+SECT_MASTER_CULT[s.masterRealm+1])+'</span></div></div>';
  html += sectRenderRes(s);
  html += sectRenderNodes(s);
  // 叙事节拍：每步一句氛围引子（不剧透），强化「逐步推进」的沉浸感
  html += '<div class="sect-mood">'+sectMoodLine(s)+'</div>';

  // 章内"节"册首语卡片（每进入一册弹一次，读后翻页即隐）
  if(s.prologue){
    html += '<div class="sect-prologue">'
      + (s.prologue.img ? '<img class="sect-prologue-img" src="assets/sect/'+s.prologue.img+'.jpg" onerror="this.style.display=\'none\'">' : '')
      + '<div class="pl-sub">'+s.prologue.sub+'</div>'
      + '<div class="pl-title">'+s.prologue.title+'</div>'
      + '<p class="pl-body">'+s.prologue.body+'</p>'
      + '<div class="pl-turn"><button class="sect-btn" onclick="sectClosePrologue()">翻过此页 →</button></div>'
      + '</div>';
  }

  // 主舞台：本旬之事（核心交互，始终可见；其余系统收进「山务」抽屉）
  html += '<div class="sect-panel sect-stage" style="margin-bottom:14px;"><h3>本旬之事 · 待你决断</h3>'+sectRenderEvents(s)+'</div>';

  // 行动栏：只留主入口「山务」与「过此旬」
  html += '<div class="sect-ap">本旬行动点 <b>'+s.ap+'</b> ／ 3　·　积压未理事 <b>'+s.backlog+'</b></div>';
  html += '<div class="sect-actions">'
    + '<button class="sect-btn" onclick="toggleShanwu()">山 务 ▸</button>'
    + '<button class="sect-btn" onclick="toggleShanzhi()">山 志 ▸</button>'
    + '<button class="sect-btn primary" onclick="sectSettle()">过 此 旬 →</button>'
    + '</div>';

  // 山务抽屉：默认收起，点击「山务」才展开。
  // 建筑/弟子/飞剑/纪事/道契/人物志与经营动作皆在此，不铺满主舞台；
  // 道契与人物志亦在结局屏完整呈现（「可以最后看到」）。
  if(s.shanwu){
    const bieyuanLv = (s.buildings['倒悬山别院']&&s.buildings['倒悬山别院'].lv>0) ? s.buildings['倒悬山别院'].lv : 0;
    const gambBtn = bieyuanLv>0 ? '<button class="sect-btn sm" onclick="sectUI(\'gamble\')">赌 局</button>' : '';
    const curR = s.masterRealm;
    const nextNeed = curR>=SECT_REALMS.length-1 ? null : SECT_MASTER_CULT[curR+1];
    const canBreak = nextNeed!=null && s.masterCult>=nextNeed && s.ap>0;
    const breakTxt = nextNeed==null ? '已 至 绝 顶' : ('破 境（'+SECT_REALMS[curR+1]+'）');
    const breakBtn = '<button class="sect-btn sm" onclick="sectBreak()" '+(canBreak?'':'disabled')+'>'+breakTxt+'</button>';

    html += '<div class="sect-drawer-mask" onclick="toggleShanwu()"></div>';
    html += '<div class="sect-drawer">'
      + '<div class="sect-drawer-h"><span>山 务</span><button class="sect-btn ghost sm" onclick="toggleShanwu()">收起 ✕</button></div>';

    // 经营动作
    html += '<div class="sect-actions sect-mg-actions">'
      + '<button class="sect-btn sm" onclick="sectUI(\'build\')">营 建</button>'
      + '<button class="sect-btn sm" onclick="sectRecruitSeek()">寻 访 弟 子</button>'
      + '<button class="sect-btn sm" onclick="sectUI(\'forge\')">祭 炼 飞 剑</button>'
      + '<button class="sect-btn sm" onclick="sectPreach()">讲 道</button>'
      + '<button class="sect-btn sm" onclick="sectTrade()">走 商</button>'
      + breakBtn + gambBtn
      + '</div>';

    // 子视图或总览
    if(s.ui==='build'){
      html += '<div class="sect-panel"><h3>营 建（升一级耗 1 行动点）</h3>'+sectRenderBuildings(s)
            + '<div style="margin-top:10px"><button class="sect-btn ghost sm" onclick="sectUI(\'main\')">返回</button></div></div>';
    } else if(s.ui==='forge'){
      html += '<div class="sect-panel"><h3>祭 炼 飞 剑（耗 灵10 银5，1 行动点）</h3>'
            + '<p class="sect-card-sub">剑庐等级越高，可祭炼柄数越多、越不易失剑。可「以道心护剑」降失败率。</p>'
            + sectRenderSwords(s)
            + '<div style="margin-top:10px"><button class="sect-btn sm" onclick="sectForge()">祭 炼 一 次</button> '
            + '<button class="sect-btn ghost sm" onclick="sectUI(\'main\')">返回</button></div></div>';
    } else if(s.ui==='gamble'){
      html += '<div class="sect-panel"><h3>倒悬山赌局（耗 1 行动点）</h3>'
        + '<p class="sect-card-sub">小赌怡情，大赌伤身。别院越高，逢凶化吉之机越盛。</p>'
        + '<div class="sect-actions">'
        + '<button class="sect-btn" onclick="sectGamble(false)">小 赌（银10）</button>'
        + '<button class="sect-btn" onclick="sectGamble(true)">孤 注（银25）</button>'
        + '<button class="sect-btn ghost sm" onclick="sectUI(\'main\')">返回</button>'
        + '</div></div>';
    } else {
      html += '<div class="sect-grid">'
        + '<div class="sect-panel"><h3>山 中 建 筑</h3>'+sectRenderBuildings(s)
        + '<div style="margin-top:8px"><button class="sect-btn sm" onclick="sectUI(\'build\')">管 理 营 建</button></div></div>'
        + '<div class="sect-panel"><h3>门 下 弟 子（'+sectSlotUsed(s)+'/'+sectSlotMax(s)+'）</h3>'+sectRenderDisciples(s)+'</div>'
        + '<div class="sect-panel"><h3>本 命 飞 剑</h3>'+sectRenderSwords(s)
        + '<div style="margin-top:8px"><button class="sect-btn sm" onclick="sectUI(\'forge\')">祭 炼</button></div></div>'
        + '<div class="sect-panel"><h3>山 中 纪 事</h3>'+sectRenderLog(s)+'</div>'
        + '</div>';
      html += sectRenderDaoqi(s);
      html += sectRenderFigures(s);
    }
    html += '</div>';
  }

  // 山志·原著脉络抽屉（默认收起，点击「山志」展开）
  if(s.chronicle) html += sectRenderChronicle(s);

  html += '<div class="sect-back"><button class="sect-btn ghost" onclick="state={phase:\'title\',sel:[],log:[]};render()">回到卷首</button></div>';
  html += '</div>';
  app.innerHTML = html;
  sectSpawnFx(s);
  sectWireFx();
...
function sectUI(ui){
  const s = state.sect; if(s.over) return;
  s.ui = ui; SFX.click(); render();
}
function toggleShanwu(){
  const s = state.sect; if(s.over) return;
  s.shanwu = !s.shanwu; SFX.click(); render();
}
function toggleShanzhi(){
  const s = state.sect; if(s.over) return;
  s.chronicle = !s.chronicle; if(s.chronicle) s.shanwu = false; SFX.click(); render();
}
function sectRenderChronicle(s){
  let h = '<div class="sect-drawer-mask" onclick="toggleShanzhi()"></div>';
  h += '<div class="sect-drawer">'
    + '<div class="sect-drawer-h"><span>山 志 · 原 著 脉 络</span><button class="sect-btn ghost sm" onclick="toggleShanzhi()">收起 ✕</button></div>'
    + '<p class="sect-card-sub" style="margin-bottom:12px">自泥瓶巷到万山朝奉，脉络依《剑来》原著按时序排列；「册」指浙江文艺出版社实体书册目。</p>'
    + '<div class="sect-chron">';
  SECT_CHRONICLE.forEach(c=>{
    h += '<div class="sect-chron-i">'
      + '<div class="sect-chron-dot"></div>'
      + '<div class="sect-chron-t">'+c.t+'</div>'
      + '<div class="sect-chron-d">'+c.d+'</div>'
      + (c.q ? '<div class="sect-chron-q">「'+c.q+'」</div>' : '')
      + '<div class="sect-chron-src">'+c.src+'</div>'
      + '</div>';
  });
  h += '</div></div>';
  return h;
