...
Page({
  ...
  onLoad(){
    const cards = WSP_CARDS.map(function(c){
      ...
      return { n:c.n, no:WSP_NUM[c.n-1], t:c.t, b:c.b||'', src:c.src,
               ...
               /* 模糊立绘（做底）：引用 /assets/portraits/<w>.jpg；无 w → 不渲染 */
               art: c.w ? ('/assets/portraits/' + c.w + '.jpg') : '', on:false };
    });
  ...
  draw(){
    ...
    const sp = wspSplit(c.t);
    const sp2 = c.b ? wspSplit(c.b) : { cols: [] };
    const detail = { n:c.n, no:WSP_NUM[c.n-1], t:c.t, b:c.b, src:c.src, w:c.w, s2:!!c.b, art:c.w ? ('/assets/portraits/' + c.w + '.jpg') : '', show2:false, hasArt: !!c.w, who: c.w ? wspWho(c.w) : '', cols: sp.cols, cols2: sp2.cols };
